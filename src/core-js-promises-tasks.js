...
function getAllResult(promises) {
  return Promise.all(
    promises.map((prom) => prom.then((value) => value).catch(() => null))
  );
}
...
function queuPromises(promises) {
  return promises.reduce(async (acc, curVal) => {
    const accCur = await acc;
    const currentVal = await curVal;
    return accCur + currentVal;
  }, '');
}
